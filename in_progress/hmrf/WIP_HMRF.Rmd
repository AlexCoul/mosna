...
```
It appears `viewHMRFresults_V2è doesn't add the results correctly to the Giotto object.
...
phillips = as.data.table(read_xlsx('/home/alexis/Pro/Postdoc_CRCT/Projects/mosna_benchmark/data/raw/CODEX_CTCL/41467_2021_26974_MOESM3_ESM_-_Objects.xlsx', skip=2))
head(phillips)
...
```{r}
coords <- matrix(runif(n=10, min=1, max=20), nrow=5)
markers <- matrix(runif(n=30, min=1, max=20), nrow=5)

instrs = createGiottoInstructions(save_plot = FALSE,
                                  show_plot = FALSE,
                                  save_dir = './temp/',
                                  python_path = NULL)
giotto = createGiottoObject(expression = markers,
                            expression_matrix_class = "custom",
                            expression_feat = "protein",
                            spatial_locs = coords,
                            instructions = instrs)
